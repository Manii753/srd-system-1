...
// Matches the dynamic-field entry that represents the "Brand" value.
// Some SRDs store it under a "Buyer" field, so both are matched here.
const BRAND_FIELD_MATCH = {
  $or: [
...
// Group all in-use brand values case-insensitively.
...
// Rename / merge / delete brand values across every SRD.
// Body: { values: string[], newValue: string }
//   - values: exact (or case-insensitive) brand values to rewrite
...
export async function POST(request) {
  ...
  if (rawValues.length === 0) {
    return NextResponse.json({ success: false, error: 'Provide at least one existing brand value' }, { status: 400 });
  }
